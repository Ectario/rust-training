...
pub fn get_battle_from_files(input_file_path: &str, output_file_path: &str) -> bool {
    ...
    if let Some(first_line) = lines.next() {
        ...
        max_turns = parse_value(values[2], "max turns");
    ...
    for line in lines {
        ...
        let fragments_by_turn: Vec<usize> = if nb_earning_turns > 0 {
            values[4..]
                .iter()
        ...
        let created_demon = Demon::new(id,
            cost,
            recovery_turns,
            recovery_stamina,
            nb_earning_turns,
